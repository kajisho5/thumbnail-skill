...
| `adapter.py` | `FfmpegSkill`: the only place a subprocess is started. Locates an ffmpeg-skill checkout, calls `probe` and `look --at --no-timecode` with typed argv, parses their JSON. A non-zero exit or `{"status": "failed"}` is `TOOL_ERROR`; a reported success whose output file doesn't actually exist (ffmpeg-skill/look can claim success for a timestamp past the last decodable frame) is `INVALID_TIME_RANGE` instead — that outcome is a fact about the timestamp, not a transient tool problem, so it is never retryable. |
